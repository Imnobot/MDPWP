jQuery(document).ready(function($) {
    ...
    $('#bigPlayImgSrc_field').remove()



    var shortcode_all_atts;

    $('#mvp_shortcode_player, #mvp_shortcode_playlist, #mvp_shortcode_ad').on('change',function(){

        var player_id = $('#mvp_shortcode_player').val(), 
        playlist_id = $('#mvp_shortcode_playlist').val(),
        ad_id = $('#mvp_shortcode_ad').val();
        
        if(!player_id){
            $('#mvp_shortcode_generator').val('Please create a player first!\n');
        }else if(!playlist_id){
            $('#mvp_shortcode_generator').val('Please create a playlist first!\n');
        }else{
            var shortcode = '[apmvp player_id="'+player_id+'" playlist_id="'+playlist_id+'"';
            if(typeof ad_id !== 'undefined' && ad_id != '')shortcode += ' ad_id="'+ad_id+'"';
            shortcode += ']';

            $('#mvp_shortcode_generator').val(shortcode);
        }
        ...
        $('#mvp_shortcode_for_php').val(shortcode_for_php);


        //get all shortcode atts
        if($(this).attr('id') == 'mvp_shortcode_player'){
            ...
            $.ajax({
                ...
                for (var [key, value] of Object.entries(response)) { 
                    ...
                    if(key != 'player_id' && 
                        key != 'elementsVisibilityArr' && 
                        key != 'mvp_edit_settings_nonce_field' && 
                        key != 'ev' && 
                        key != 'playbackPositionKey' && 
                        key != 'sourcePath' && 
                        key != 'playbackRateArr' && 
                        key != 'caption_breakPointArr' && 
                        key != 'breakPointArr' && 
                        key != 'keyboardControls' && 
                        key.indexOf('keyboard') == -1 && 
                        key != 'hlsConfig' && 
                        key != 'dashConfig' && 
                        key != 'presets' && 
                        key.indexOf('Arr') == -1 && 
                        key != 'media_end_action_html' && 
                        key != 'media_end_action_css' && 
                        key.indexOf('Icon') == -1){
                ...
                $('#mvp_shortcode_generator_all_atts').val(str);
        ...
        else if($(this).attr('id') == 'mvp_shortcode_ad'){
            ...
            if(shortcode_all_atts){
                if(typeof ad_id !== 'undefined' && ad_id != ''){
                    ...
                    $('#mvp_shortcode_generator_all_atts').val(s3);

                }else if(ad_id == ''){//remove ad_id

                    var s = $('#mvp_shortcode_generator_all_atts').val();
    ...
    $('#mvp_shortcode_player').change();




    //remove lock time from quick shortcode generator
    $(".mvp-quick-shortcode-field").find("#lock_time_field").remove()
    $(".mvp-quick-shortcode-field").find("#playlist_icons_field").remove()


    //quick video generator

    var playlistGridStyleVal;
    shortcodeManagerSection.find('#playlistGridStyle').on('change',function(){
    ...
    shortcodeManagerSection.find('#playlistStyle').on('change',function(){
    ...
    shortcodeManagerSection.find('#playlistPosition').on('change',function(){
